...
pub async fn load_native(core: &mut ArmCore, system: &mut System, jvm: &Jvm, jar_path: &str, data: &[u8]) -> Result<()> {
    ...
    tracing::info!(
        "[PHASE8_55_LGT_NATIVE_ENTRY] aid={} pid={} entrypoint={entrypoint:#010x} init_param1={ptr_init_param_1:#010x} init_param2={ptr_init_param_2:#010x}",
        system.aid(),
        system.pid()
    );
    ...
    if let Err(error) = &entry_result {
        tracing::error!("[PHASE8_55_LGT_ENTRY_ERROR] aid={} pid={} error={error}", system.aid(), system.pid());
    }
    entry_result?;
    ...
    tracing::info!(
        "[PHASE8_55_LGT_INIT_STRUCT] aid={} pid={} ptr={:#010x} fn_init={:#010x}",
        system.aid(),
        system.pid(),
    ...
    if let Err(error) = &init_result {
        tracing::error!(
            "[PHASE8_55_LGT_INITIALIZER_ERROR] aid={} pid={} fn_init={:#010x} error={error}",
            system.aid(),
            system.pid(),
            init_struct.fn_init
        );
    ...
    init_result?;
    tracing::info!("[PHASE8_55_LGT_INITIALIZER_COMPLETE] aid={} pid={}", system.aid(), system.pid());
...
async fn get_import_function(core: &mut ArmCore, import_table: u32, function_index: u32) -> Result<u32> {
    ...
    Ok(match (import_table, function_index) {
        ...
        _ => {
            tracing::error!(
                "[PHASE8_55_LGT_UNSUPPORTED_IMPORT] table={import_table:#x} function_index={function_index:#x}"
            );
