...
fn trace_unknown_wipic(core: &ArmCore, id: u32) {
    let ctx = core.save_context();
    tracing::error!(
        "[LGT_ABI] unknown WIPIC id={:#x} ({}) pc={:#x} lr={:#x} sp={:#x} cpsr={:#x} r0={:#x} r1={:#x} r2={:#x} r3={:#x} r4={:#x} r5={:#x} r6={:#x} r7={:#x} r8={:#x} r9={:#x} r10={:#x} r11={:#x} r12={:#x}",
        id, id, ctx.pc, ctx.lr, ctx.sp, ctx.cpsr, ctx.r0, ctx.r1, ctx.r2, ctx.r3, ctx.r4, ctx.r5, ctx.r6, ctx.r7, ctx.r8, ctx.sb, ctx.sl, ctx.fp, ctx.ip
    );
    ...
    tracing::error!("[LGT_ABI] WIPIC id={id:#x} stack_words={stack_words:#x?}");
    ...
    tracing::error!("[LGT_ABI] WIPIC id={id:#x} caller={caller:#x} caller_words_base={base:#x} words={caller_words:#x?}");
    ...
    for (name, ptr) in [("r0", ctx.r0), ("r1", ctx.r1), ("r2", ctx.r2), ("r3", ctx.r3)] {
        if ptr >= 0x10000 {
            ...
            if ok {
                tracing::error!("[LGT_ABI] WIPIC id={id:#x} {name}_preview ptr={ptr:#x} words={words:#x?}");
            }
        }
    }
}
