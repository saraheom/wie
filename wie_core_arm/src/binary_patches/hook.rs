...
pub enum HookKind {
    ...
    /// Phase 8.56 — production Inotia1 monster base-reward overflow repair using the
...
async fn handle_binary_patch_svc(core: &mut ArmCore, registry: &mut Registry) -> Result<JumpTo> {
    ...
    match kind {
        ...
        HookKind::Inotia1RewardWideMathRepair => {
            ...
            // Phase 8.56 cleanup: normal monster spawns are intentionally silent.
            ...
            if repaired {
                tracing::info!(
                    "[PHASE8_56_INOTIA1_REWARD_OVERFLOW_REPAIR] lr={lr:#010x} original={} corrected={} original_hex={:#010x} corrected_hex={:#010x} wide_numerator={} wide_denominator={} a={a} b={b} c={c} d={d} e={e} f={f}",
                    original_value,
                    corrected_value,
